...
R package "bifaToolkits" available: [https://mavis-liang.github.io/bifaToolkits/](https://mavis-liang.github.io/bifaToolkits/). Navigate to the "Article" Tab for demos.
...
- main_code
  ...
  - `plot_box.r` and `tab_NumFactor.R`: the code that generates the box plots and table for numbers of factors to display the simulation results shown in the manuscript. `plot_sc5.r` makes figures for Scenario 5---results of this scenario are stored differently (each method has an individual RDS) because results are large, and therefore, different operation is needed to plot.
