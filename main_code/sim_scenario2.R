...
# --------------------------- Scenario BMSFA (scenario 2)---------------------------
# This function produce the result of a single simulation of the scenario 2
...
source("./functions/gen_scenario2.R")
sim_scenario2 <- function(seed=1){
  set.seed(seed)
  # Data generation
  true_data <- gen_scenario2(S=4, N_s=c(100, 100, 100, 100), P=40, K=4, c(1,1,1,1))
  # Centering the data
  ...
  profile_indFA <- peakRAM::peakRAM({
    fit_indFA <-
      lapply(1:length(true_data$N_s), function(s){
        j_s = c(5, 5, 5, 5)
        MSFA::sp_fa(true_data$Y_list[[s]], k = j_s[s], scaling = FALSE, centering = TRUE, trace = FALSE,
                    control = list(nrun = 10000, burn = 8000))
      })
  })
  ...
  profile_df <- rbind(profile_stackFA[,c(2, 4)], profile_indFA[,c(2, 4)], 
    ...
    t() %>% `colnames<-`(c("stackFA", "IndFA", "MOMSS", "BMSFA",
                           "PFA", "SUFA", "SUFA_fixJs", "Tetris_fixT"))
