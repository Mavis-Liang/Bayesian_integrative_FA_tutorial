...
plot_covs <- function(fitted_lists, sim_data) {
  ...
  lambda_data$CovType <- factor(lambda_data$CovType, labels = sapply(unique(lambda_data$CovType), function(i) TeX(paste0("$\\Sigma_{\\Lambda_", gsub("SigmaLambda_", "", i), "}$"))))
  marginal_data$CovType <- factor(marginal_data$CovType, labels = sapply(seq_along(unique(marginal_data$CovType)), function(i) TeX(paste0("$\\Sigma_{", i, "}$"))))
  ...
  marginal_data$Method <- factor(marginal_data$Method, levels = c("True", names(fitted_lists)))
  ...
  p_phi <- ggplot(phi_data, aes(x = Var2, y = Var1, fill = value)) +
    ...
    scale_fill_gradient2(low = "purple", mid = "white", high = "green", midpoint = 0, na.value = "grey80", name = TeX("$\\Sigma_{\\Phi}$\\ values")) +
  ...
  p_lambda <- ggplot(lambda_data, aes(x = Var2, y = Var1, fill = value)) +
    ...
    scale_fill_gradient2(low = "lightblue", mid = "white", high = "red", 
                         midpoint = 0, na.value = "grey80", name = TeX("$\\Sigma_{\\Lambda}$\\ values")) +
    guides(fill = guide_colorbar(barwidth = 0.5, barheight = 4)) +
...
arrange_SigmaLambda <- function(fitted_lists, sim_data) {
  ...
  lambda_melted_list <- lapply(c("True", names(fitted_lists)), function(method) {
    if (method == "True") {
...
arrange_SigmaMarginal <- function(fitted_lists, sim_data) {
  ...
  marginal_melted_list <- lapply(c("True", names(fitted_lists)), function(method) {
    if (method == "True") {
